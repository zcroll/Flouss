...
const { activeTab, handleTabChange, handleSidebarNavigation } = useAdminTabs()

// Initialize the active tab and sidebar state on mount
onMounted(() => {
  const path = window.location.pathname
  handleSidebarNavigation(path)
  ...
  if (path === '/adminn/dashboard') {
    ...
    handleTabChange(segment === 'dashboard' ? 'overview' : segment)
  }
})

// Watch for active tab changes
watch(activeTab, (newTab) => {
  const title = newTab.charAt(0).toUpperCase() + newTab.slice(1)
  document.title = `Dashboard ${title}`
})
</script>

<template>
  <Head :title="`Dashboard ${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}`" />
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          ...
          <main class="flex-1 overflow-x-hidden overflow-y-auto bg-background">
            <div class="container mx-auto px-6 py-8">
              ...
              <AdminTabs>
                <template #content="{ activeTab: currentTab }">
                  <!-- Overview Tab -->
                  <TabsContent 
                    value="overview" 
                    :class="['space-y-6', { 'hidden': currentTab !== 'overview' }]"
                  >
                    <!-- Stats Cards -->
                  ...
                  <TabsContent 
                    value="analytics" 
                    :class="['space-y-6', { 'hidden': currentTab !== 'analytics' }]"
                  >
                    <!-- Stats Cards -->
                    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                      <Card>
                        ...
                        <CardContent>
                          <div class="text-2xl font-bold">15,234</div>
                          <p class="text-xs text-muted-foreground">+25.1% from last month</p>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent>
                          <div class="text-2xl font-bold">+573,893</div>
                          <p class="text-xs text-muted-foreground">+19% from last month</p>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent>
                          <div class="text-2xl font-bold">+2,234</div>
                          <p class="text-xs text-muted-foreground">+201 since last hour</p>
                        </CardContent>
                      </Card>
                    </div>
                  ...
                  <TabsContent 
                    value="reports" 
                    :class="['space-y-6', { 'hidden': currentTab !== 'reports' }]"
                  >
                    <!-- Stats Cards -->
                    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                      <Card>
