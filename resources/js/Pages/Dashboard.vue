    ...
    <AppLayout :name-exist="true" :it-is-visible="true">
        <div class="min-h-screen">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
                <!-- Welcome Banner -->
                <div class="bg-stone-950 rounded-2xl p-4 sm:p-8 mb-4 border border-stone-700">
                    <div class="flex flex-col md:flex-row justify-between items-center">
                        <div>
                            <h1 class="text-2xl sm:text-4xl font-bold text-stone-100 text-center md:text-left">
                                {{ __('dashboard.welcome') }} {{ $page.props.auth.user.name }}!
                            </h1>
                            <p class="mt-2 text-base sm:text-lg text-stone-400 text-center md:text-left">
                        ...
                        <div class="mt-4 md:mt-0">
                            <Link v-if="!hasResult"
                                href="/main-test"
                                class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-amber-500 hover:bg-amber-600 text-stone-900 text-sm sm:text-base font-medium rounded-lg transition-colors duration-200"
                            >
                ...
                <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        ...
                        <div class="bg-stone-950 rounded-2xl p-4 sm:p-6 border border-stone-700">
                            <div class="flex items-center justify-between mb-4 sm:mb-6">
                                <h2 class="text-xl sm:text-2xl font-semibold text-stone-100">{{ __('dashboard.liked_jobs') }}</h2>
                                <span class="px-3 sm:px-4 py-1.5 sm:py-2 bg-stone-700 rounded-full text-xs sm:text-sm text-stone-300">
                                    {{ favoriteJobs.length }}
                                </span>
                            </div>
                            <div v-if="favoriteJobs.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-h-[200px] overflow-y-auto custom-scrollbar">
                                <div v-for="job in favoriteJobs" :key="job.id"
                                    ...
                                    <Link :href="`/career/${job.slug}`" class="flex items-center justify-between">
                                        <div>
                                            <h3 class="text-base sm:text-lg font-medium text-stone-100">{{ job.name }}</h3>
                                            <p class="text-xs sm:text-sm text-stone-400">Career Path</p>
                                        </div>
                            ...
                            <div v-else class="flex flex-col items-center justify-center py-6 sm:py-8 text-center">
                                ...
                                <p class="text-sm sm:text-base text-stone-400 mb-2">{{ __('dashboard.no_liked_jobs') }}</p>
                                <Link href="/jobs" class="text-xs sm:text-sm text-amber-500 hover:text-amber-400">
                        ...
                        <div class="bg-stone-950 rounded-2xl p-4 sm:p-6 border border-stone-700">
                            ...
                            <div v-if="favoriteDegrees.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-h-[200px] overflow-y-auto custom-scrollbar">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div v-if="hasResult" class="lg:col-span-2 bg-stone-950 rounded-2xl p-4 sm:p-6 border border-stone-700">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
                            <h2 class="text-xl sm:text-2xl font-semibold text-stone-100">{{ __('dashboard.your_assessment_results') }}</h2>
                            <Link
                                href="/results"
                                class="w-full sm:w-auto px-3 py-1.5 text-xs sm:text-sm bg-white hover:bg-stone-300/30 hover:text-white text-stone-900 rounded-lg transition-colors duration-200 text-center"
                            >
                    ...
                    <!-- AI Assistant Card -->
                    <div class="bg-stone-950 rounded-2xl border border-stone-700 flex flex-col h-[400px] sm:h-[600px]">
                        <div class="p-4 sm:p-6 border-b border-stone-700">
                            <div class="flex items-center space-x-3">
                                <div class="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-amber-500 rounded-full animate-pulse"></div>
                                <h2 class="text-lg sm:text-xl font-semibold text-stone-100">{{ __('dashboard.orientation_ai_assistant') }}</h2>
                            </div>
                        </div>
                        <div class="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar">
                            <div class="space-y-4">
                                <div class="bg-black rounded-xl p-3 sm:p-4">
                                    <div class="flex items-center space-x-2 mb-2 sm:mb-3">
                                        <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-amber-500/20 flex items-center justify-center">
                                        ...
                                        <span class="text-sm sm:text-base text-stone-300 font-medium">AI Assistant</span>
                                    </div>
                                    <p class="text-xs sm:text-sm text-stone-300 leading-relaxed animate-fade-in">{{ displayedText }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
